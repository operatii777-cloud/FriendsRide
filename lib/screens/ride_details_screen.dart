...
class _RideDetailsScreenState extends State<RideDetailsScreen> {
  ...
  Future<void> _updateMarkersAndCamera() async {
    ...
    double minLat = points.map((p) => p.coordinates.lat).reduce((a, b) => a < b ? a : b).toDouble();
    double maxLat = points.map((p) => p.coordinates.lat).reduce((a, b) => a > b ? a : b).toDouble();
    double minLng = points.map((p) => p.coordinates.lng).reduce((a, b) => a < b ? a : b).toDouble();
    double maxLng = points.map((p) => p.coordinates.lng).reduce((a, b) => a > b ? a : b).toDouble();
  ...
  Future<void> _getCoordinatesFromAddresses() async {
    try {
      List<Location> startLocations = await locationFromAddress(widget.ride.startAddress);
      List<Location> endLocations = await locationFromAddress(widget.ride.destinationAddress);
