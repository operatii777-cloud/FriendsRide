  ...
  void _viewRoute() async {
    try {
      ...
      final route = await RoutingService().getRoute([
        Point(coordinates: Position(_currentPosition!.longitude, _currentPosition!.latitude)),
        Point(coordinates: Position(widget.ride.startLongitude!, widget.ride.startLatitude!)),
        Point(coordinates: Position(widget.ride.destinationLongitude!, widget.ride.destinationLatitude!)),
      ]);
      
      if (!mounted) return;
      
      // Navighează la ActiveRideScreen cu preview
      Navigator.push(
        context,
        MaterialPageRoute(
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Eroare la încărcarea rutei: $e')),
      );
    }
  }
  ...
  void _startRide() async {
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('❌ Eroare la pornirea cursei'),
            backgroundColor: Colors.red,
          ),
        );
      }
        ...
        if (_pickupState == PickupState.pickingUp) ...[
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: const Text('Pornește cursa'),
              style: ElevatedButton.styleFrom(
                backgroundColor: Colors.green,
                foregroundColor: Colors.white,
