...
class CancellationPolicy {
  ...
  static CancellationPolicy calculatePolicy({
    ...
    if (isScheduled && scheduledPickupTime != null) {
      final timeUntilPickup = scheduledPickupTime.difference(now);
      
      // Anulare gratuită dacă mai sunt cel puțin 1.5 ore (90 minute) până la pickup
      if (timeUntilPickup.inMinutes >= 90) {
        return const CancellationPolicy(
          isFree: true,
          reason: 'Anulare gratuită (cu cel puțin 1 oră și 30 de minute înainte de ora programată)',
          appliesToDriver: true,
          appliesToPassenger: true,
        );
      }
      
      // Taxă dacă mai sunt sub 1.5 ore până la pickup
      if (timeUntilPickup.inMinutes < 90 && timeUntilPickup.inMinutes > 0) {
        ...
        final fee = math.min(30.0, rideCost);
        return CancellationPolicy(
          isFree: false,
          fee: fee,
          reason: 'Taxă de anulare: ${fee.toStringAsFixed(2)} RON (30 RON sau prețul cursei - valoarea cea mai mică)',
