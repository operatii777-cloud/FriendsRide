...
class OptimizedGeocodingService {
  ...
  Future<List<AddressSuggestion>> searchAddresses({
    ...
    try {
      ...
      // 2. Caută în multiple surse în paralel
      ...
      // 3. Procesează și sortează rezultatele
  ...
  Future<List<AddressSuggestion>> _searchMultipleSources(
    ...
    try {
      ...
      final results = await Future.wait(
        futures.map((f) => f.timeout(Duration(milliseconds: 500), onTimeout: () => <AddressSuggestion>[])),
      );
  ...
  List<AddressSuggestion> _parseNominatimResults(
    ...
    for (final item in data) {
      ...
      suggestions.add(AddressSuggestion(
        ...
        latitude: lat,
        longitude: lon,
        score: 0, // Va fi calculat mai târziu
      ));
    }
