...
class PoiService {
  ...
  final List<PointOfInterest> _staticPois = [
    PointOfInterest(
      ...
      imageUrl: 'https://placehold.co/600x400/9C27B0/FFFFFF?text=🏛️+Palatul+Parlamentului',
      location: Position(latitude: 44.4275, longitude: 26.0875, timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0, heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0),
    ...
    PointOfInterest(
      id: 'static_ateneu',
      name: 'Ateneul Român',
      description: 'O sală de concerte emblematică și un simbol al culturii române.',
      imageUrl: 'https://placehold.co/600x400/9C27B0/FFFFFF?text=🏛️+Ateneul+Roman',
      location: Position(latitude: 44.4415, longitude: 26.0975, timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0, heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0),
      category: PoiCategory.tourism,
      isStatic: true,
    ),
    PointOfInterest(
      ...
      description: 'Găzduit în fostul Palat Regal, expune artă românească și europeană.',
      imageUrl: 'https://placehold.co/600x400/9C27B0/FFFFFF?text=🏛️+MNAR',
      location: Position(latitude: 44.4404, longitude: 26.0963, timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0, heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0),
      category: PoiCategory.tourism,
      isStatic: true,
    ),
    PointOfInterest(
      ...
      imageUrl: 'https://placehold.co/600x400/9C27B0/FFFFFF?text=🏛️+Gradina+Botanica',
      location: Position(latitude: 44.4367, longitude: 26.0625, timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0, heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0),
      category: PoiCategory.tourism,
      isStatic: true,
    ),
    PointOfInterest(
      ...
      description: 'Cel mai mare parc din București, ideal pentru relaxare și plimbări.',
      imageUrl: 'https://placehold.co/600x400/9C27B0/FFFFFF?text=🏛️+Parcul+Herastrau',
      location: Position(latitude: 44.4715, longitude: 26.0825, timestamp: DateTime.now(), accuracy: 0, altitude: 0, altitudeAccuracy: 0, heading: 0, headingAccuracy: 0, speed: 0, speedAccuracy: 0),
      category: PoiCategory.tourism,
      isStatic: true,
    ),
  ];
  ...
  Future<List<PointOfInterest>> getPoisForArea({
    ...
    for (final poi in _localPois) {
      if (activeCategories.contains(poi.category)) {
        final d = _distanceMeters(latitude, longitude, poi.location.latitude, poi.location.longitude);
    ...
    for (final category in activeCategories) {
      if (category != PoiCategory.tourism) { // Turismul e static
    ...
    for (final p in allPois) {
      final key = '${p.location.latitude.toStringAsFixed(6)}_${p.location.longitude.toStringAsFixed(6)}_${p.category.name}';
  ...
  Future<List<PointOfInterest>> _fetchDynamicPois({
    ...
    final Future<List<PointOfInterest>> future = (() async {
      ...
      final response = await _httpGetWithRetry(url, maxRetries: 3);
      ...
      if (response.statusCode == 200) {
        ...
        final pois = features
            .map((feature) => PointOfInterest.fromMapboxFeature(feature as Map<String, dynamic>))
            .toList();
  ...
  String _calculateBbox(double lat, double lng, double radiusKm) {
    ...
    final double deltaLng = radiusKm / earthRadius * (180 / math.pi) / math.cos(latRad);
  ...
  PointOfInterest _poiFromDiskJson(Map<String, dynamic> m) {
    ...
    return PointOfInterest(
      id: (m['id'] ?? 'poi_${DateTime.now().millisecondsSinceEpoch}').toString(),
  ...
  Future<void> loadLocalGeoJsonFromAsset(String assetPath) async {
    ...
    try {
      ...
      for (final f in features) {
        ...
        final poiCategory = _mapOsmToCategory(amenity: amenity, tourism: tourism, shop: shop);
        ...
        final poi = PointOfInterest(
          id: (f['id']?.toString() ?? 'local_geojson_${DateTime.now().millisecondsSinceEpoch}_$index'),
          name: name == null || name.isEmpty ? 'POI' : name,
          description: (props['address'] ?? props['addr:full'] ?? props['opening_hours'] ?? '')
              .toString(),
          imageUrl: _generateImageUrl(poiCategory, name ?? 'POI'),
          location: Position(
            latitude: lat,
            longitude: lng,
            timestamp: DateTime.now(),
  ...
  Future<void> loadLocalGpxFromAsset(String assetPath) async {
    ...
    try {
      final content = await rootBundle.loadString(assetPath);
      final wptExp = RegExp(r'<wpt[^>]*lat="([^"]+)"[^>]*lon="([^"]+)"[^>]*>([\s\S]*?)<\/wpt>', multiLine: true);
      ...
      for (final m in matches) {
        ...
        final poi = PointOfInterest(
          ...
          additionalInfo: {
            ...
            if (kv.containsKey('addr:housenumber')) 'addr:housenumber': kv['addr:housenumber'],
            ...
            if (kv.containsKey('opening_hours')) 'opening_hours': kv['opening_hours'],
          },
        );
  ...
  Future<void> loadLocalKmlFromAsset(String assetPath) async {
    ...
    try {
      ...
      final placemarkExp = RegExp(r'<Placemark[\s\S]*?<name>([\s\S]*?)<\/name>[\s\S]*?<Point>[\s\S]*?<coordinates>([\s\S]*?)<\/coordinates>[\s\S]*?<\/Point>[\s\S]*?<\/Placemark>', multiLine: true);
      ...
      for (final m in matches) {
        ...
        final firstTuple = coords.split(RegExp(r'\s+')).firstWhere((e) => e.contains(','), orElse: () => '');
        ...
        String? extractField(String key) {
          final r = RegExp('<Data name="${RegExp.escape(key)}"><value>([\\s\\S]*?)<\\/value><\\/Data>');
  ...
  List<double>? _centroidFromMultiPolygon(List<dynamic> multiPolygonCoords) {
    // multiPolygonCoords: [ polygon, polygon, ... ]
  ...
  PoiCategory _mapOsmToCategory({String? amenity, String? tourism, String? shop}) {
    final a = (amenity ?? '').toLowerCase();
    final t = (tourism ?? '').toLowerCase();
    final s = (shop ?? '').toLowerCase();

    if (a == 'fuel') return PoiCategory.gasStation;
    if (a == 'restaurant' || a == 'fast_food' || a == 'cafe' || a == 'food_court') return PoiCategory.restaurant;
    if (a == 'parking' || a == 'parking_entrance' || a == 'parking_space') return PoiCategory.parking;
    if (a == 'hospital' || a == 'clinic') return PoiCategory.hospital;
    if (a == 'pharmacy') return PoiCategory.pharmacy;
    if (a == 'hotel' || t == 'hotel' || s == 'hotel') return PoiCategory.hotel;
    if (s == 'supermarket' || s == 'grocery' || s == 'convenience') return PoiCategory.supermarket;
    if (a == 'bank') return PoiCategory.bank;
    if (a == 'atm') return PoiCategory.atm;
    if (a == 'school' || a == 'kindergarten') return PoiCategory.school;
    if (a == 'university' || a == 'college') return PoiCategory.university;
    if (a == 'library') return PoiCategory.library;
    if (a == 'police') return PoiCategory.police;
    if (a == 'post_office') return PoiCategory.postOffice;
    if (a == 'shopping_mall' || a == 'mall' || a == 'marketplace') return PoiCategory.mall;
    if (s == 'bakery' || a == 'bakery') return PoiCategory.bakery;
    if (a == 'bar' || a == 'pub') return PoiCategory.barPub;
    if (a == 'park') return PoiCategory.park;
    if (a == 'museum') return PoiCategory.museum;
    if (a == 'cinema') return PoiCategory.cinema;
    if (a == 'theatre') return PoiCategory.theatre;
    if (a == 'playground') return PoiCategory.playground;
    if (a == 'charging_station') return PoiCategory.chargingStation;
    if (a == 'car_wash') return PoiCategory.carWash;
    if (a == 'car_repair') return PoiCategory.carRepair;
    if (a == 'bus_stop' || a == 'tram_stop' || a == 'train_station' || a == 'subway_entrance' || a == 'public_transport') return PoiCategory.publicTransport;
    if (a == 'aerodrome' || a == 'airport') return PoiCategory.airport;
    if (t.isNotEmpty) return PoiCategory.tourism;
    return PoiCategory.other; // Fallback explicit
  }
  ...
  /// Fetch POIs from Mapbox Searchbox Category API limited to Bucharest-Ilfov (cu cache + coalescing + retry)
  Future<List<PointOfInterest>> fetchPoisFromApi(
    Map<String, double> center,
    PoiCategory category,
  ) async {
    final String accessToken = MapboxConfig.getAccessToken();
    final String categoryKey = category.mapboxType; // reuse existing mapping

    // Chei calculate (rezervate pentru caching/coalescing) — în prezent nefolosite

    // Cache valid? (disabled for now)
...
List<int> poiSortByDistanceIndicesCompute(Map<String, dynamic> args) {
  final List<dynamic> rawCoords = (args['coords'] as List<dynamic>? ) ?? const [];
  ...
  final List<int> result = List<int>.generate(take, (i) => distances[i].index, growable: false);
  return result;
}
...
// Coordonate aproximative pentru București și Județul Ilfov
// Format: minLongitudine,minLatitudine,maxLongitudine,maxLatitudine
const String _bucharestIlfovBbox = '25.75,44.20,26.45,44.70';
...
PointOfInterest? _poiFromSearchboxFeature(Map<String, dynamic> feature) {
  ...
  final String name = (feature['name'] ?? properties?['name'] ?? properties?['full_address'] ?? 'POI').toString();
  final String address = (properties?['full_address'] ?? properties?['place_formatted'] ?? '').toString();
  final String id = (feature['id'] ?? properties?['mapbox_id'] ?? 'poi_${DateTime.now().millisecondsSinceEpoch}').toString();

  // Category hint from properties if available; fallback to requested type
  final String cat = (properties?['category'] ?? categoryFromProperties(properties)).toString();
  final PoiCategory poiCategory = PoiCategoryExtension.mapboxTypeToCategory(cat);

  return PointOfInterest(
    id: id,
    name: name,
    description: address.isNotEmpty ? address : 'Locație din ${poiCategory.displayName}',
  ...
  while (true) {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final List<dynamic> features = (data['features'] as List<dynamic>? ) ?? const [];
