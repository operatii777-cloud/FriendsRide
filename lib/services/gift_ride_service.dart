...
class GiftRideService {
  ...
  /// Obține lista de gift rides trimise de utilizator
  ...
  /// Obține lista de gift rides primite de utilizator (după email sau telefon)
  Future<List<GiftRide>> getUserReceivedGifts(String userId) async {
    try {
      ...
      // Caută după userId (dacă a revendicat deja)
      queries.add(
        _db
            .collection('gift_rides')
      ...
      final gifts = <GiftRide>{};

      for (final snapshot in results) {
        for (final doc in snapshot.docs) {
          gifts.add(GiftRide.fromMap(doc.data()));
        }
      }
