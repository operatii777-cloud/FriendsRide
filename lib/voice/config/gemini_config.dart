...
class GeminiConfig {
  // 🎯 API Configuration
  // ✅ FIX: Folosim API v1 cu model gemini-pro (disponibil și stabil)
  // Notă: gemini-1.5-flash nu este disponibil în v1beta pentru generateContent, folosim gemini-pro în v1
  ...
  static const bool _isProduction = false; // Setează din build config când e gata
  
  /// 🔑 Obține API key-ul pentru mediul curent
  static String get apiKey {
    // 🔍 Încearcă să citească din environment variables
    const fromEnv = String.fromEnvironment('GEMINI_API_KEY');
    if (fromEnv.isNotEmpty) {
      return fromEnv;
    }
    
    // 🎯 Fallback la configurația statică
    if (_isProduction) {
