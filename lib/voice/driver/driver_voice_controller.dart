...
class DriverVoiceController extends ChangeNotifier {
  ...
  Future<void> acceptRide() async {
    ...
    try {
      await _voice.speak("Comandă acceptată! Încep navigația către client.");
  ...
  String _generateNavigationMessage() {
    ...
    return "Perfect! Navigez către ${request.pickupAddress}. "
           "Clientul a fost anunțat. "
           "Timp estimat de sosire: ${request.etaToPickup} minute. "
           "Comenzi vocale disponibile: 'Am ajuns la client', 'Client a urcat', 'Ajutor'.";
  }
  ...
  // VOICE-GUIDED RIDE FLOW
  Future<void> _startVoiceGuidedRide() async {
    _setState(DriverVoiceState.navigatingToPickup);
    
    // Start continuous voice commands listening
    _startContinuousVoiceCommands();
  ...
  void _startContinuousVoiceCommands() async {
    while (_state != DriverVoiceState.idle && _state != DriverVoiceState.rideCompleted) {
      try {
        // Listen for "salut" or direct commands
        final command = await _voice.listen(timeoutSeconds: 5);
        
        if (command != null) {
          await _processDriverCommand(command);
        }
  ...
  Future<void> _handleEmergencyCommand(String command) async {
    ...
    final emergency = await _voice.listen(timeoutSeconds: 15);
    if (emergency != null) {
  ...
  Future<void> _handleSendMessage() async {
    ...
    final message = await _voice.listen(timeoutSeconds: 15);
  ...
  Future<void> _handleProblem() async {
    ...
    final problem = await _voice.listen(timeoutSeconds: 15);
  ...
  Future<void> _showDriverHelp() async {
    final helpMessage = "Comenzi disponibile: 'Am ajuns la client', 'Client a urcat', "
  ...
  void _startSafetyMonitoring() {
    // Monitor for long periods of silence or unusual patterns
    _safetyMonitoringTimer?.cancel();
    _safetyMonitoringTimer = Timer.periodic(Duration(seconds: 30), (timer) {
      if (_state == DriverVoiceState.idle) {
        timer.cancel();
        return;
      }
      ...
      if (_lastCommandTime != null && 
          DateTime.now().difference(_lastCommandTime!) > Duration(minutes: 5)) {
        _checkDriverStatus();
      }
    });
  }
  ...
  Future<void> _checkDriverStatus() async {
    await _voice.speak("Verific statusul șoferului. Sunteți în regulă?");
    
    final response = await _voice.listen(timeoutSeconds: 10);
    if (response == null) {
      await _voice.speak("Nu am primit răspuns. Contactez suportul pentru verificare.");
      if (_currentRequest == null) {
        return;
      }
      try {
        await _submitSupportTicket(
          reportType: 'driver_inactivity',
