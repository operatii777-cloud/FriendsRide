...
class _VoiceDemoScreenState extends State<VoiceDemoScreen> {
  ...
  void _showDriverStatistics(DriverVoiceController voiceController) {
    ...
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text('📊 Driver Voice Statistics'),
        content: Column(
  ...
  Future<void> _testMicrophone(BuildContext context) async {
    ...
    try {
      ...
      if (result != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Microphone test failed - no input detected')),
        );
      }
    } catch (e) {
      if (context.mounted) {
  ...
  Future<void> _testSpeechRecognition(BuildContext context) async {
    ...
    try {
      ...
      if (result != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Speech recognition failed - no input detected')),
        );
      }
    } catch (e) {
      if (context.mounted) {
  ...
  Future<void> _testTextToSpeech(BuildContext context) async {
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Text-to-speech test completed successfully!')),
        );
      }
    } catch (e) {
      if (context.mounted) {
