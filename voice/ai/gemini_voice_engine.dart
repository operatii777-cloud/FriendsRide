...
class GeminiVoiceEngine {
  ...
  Future<void> initialize() async {
    try {
      ...
      // ✅ FIX: Folosim 'gemini-pro' (model disponibil și stabil pentru API v1)
  ...
  // 🚗 Context specific FriendsRide (pentru viitoare implementări)
  ...
  Future<GeminiVoiceResponse> processVoiceInput(String userInput, VoiceContext context, {String? languageCode}) async {
    try {
      ...
      _model ??= GenerativeModel(model: 'gemini-pro', apiKey: GeminiConfig.apiKey);
      _chat ??= _model!.startChat();
  ...
  String _buildGeminiPromptRomanian(String userInput, VoiceContext context) {
    return '''
Ești asistentul vocal pentru FriendsRide, o aplicație de ride sharing din România.
...
''';
  ...
  String _buildGeminiPromptEnglish(String userInput, VoiceContext context) {
    return '''
You are the voice assistant for FriendsRide, a ride sharing app from Romania.
...
''';
  ...
  void _updateConversationContext(String userInput, GeminiVoiceResponse response) {
    ...
    // 🚗 Actualizez contextul FriendsRide
