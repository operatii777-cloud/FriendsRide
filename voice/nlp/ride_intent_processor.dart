...
class RideIntentProcessor {
  ...
  /// 🚀 ISOLATE OPTIMIZATION: Process intent in isolate
  static Future<RideIntent> _processIntentInIsolate(String text) async {
    // Optimized local processing for common patterns
    final intent = _processLocalPatterns(text);
    if (intent.confidence > 0.9) {
      return intent;
    }
    
    // Use Gemini for complex cases
    return await _processWithGeminiAPI(text);
  }
  ...
  /// 🤖 GEMINI AI: Advanced intent processing with Gemini API
  static Future<RideIntent> _processWithGeminiAPI(String text) async {
    try {
      ...
      // Advanced prompt for Gemini AI
      final prompt = """
      Ești un procesor de limbaj natural pentru o aplicație de ride-sharing din România, numită FriendsRide.
      ...
      if (response.statusCode == 200) {
        ...
        // FIX: Curăță JSON-ul de markdown wrapper
        ...
        return RideIntent(
          type: _parseIntentType(intentData['type']),
          destination: intentData['destination'] ?? '',
          pickupLocation: intentData['pickup'] ?? 'current_location',
          confidence: (intentData['confidence'] ?? 0.7).toDouble(),
          parameters: Map<String, dynamic>.from(intentData['parameters'] ?? {}),
        );
      } else {
