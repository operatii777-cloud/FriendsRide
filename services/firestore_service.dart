...
import 'package:friendsride_app/models/support_ticket_model.dart';
...
_pendingUpdates.addAll(updates.map((e) => e as Map<String, dynamic>));
  ...
  Stream<List<Ride>> getPendingRideRequests(RideCategory driverCategory) {
    ...
    // ✅ PERFORMANCE: Add limit and orderBy for faster queries
    return _db.collection('ride_requests')
        .where('status', whereIn: ['pending', 'driver_found'])
        .where('category', whereIn: _getVisibleCategories(driverCategory))
        .orderBy('timestamp', descending: true) // ✅ PERFORMANCE: Order by timestamp
        .limit(50) // ✅ PERFORMANCE: Limit results for faster queries
        .snapshots()
        .transform(StreamTransformer.fromHandlers(
          handleData: (QuerySnapshot snapshot, EventSink<List<Ride>> sink) {
            _rideSearchDebouncer?.cancel();
            _rideSearchDebouncer = Timer(const Duration(milliseconds: 200), () async {
              final filteredRides = await _processRidesInBackground(snapshot);
              sink.add(filteredRides);
            });
          }
        ));
  }
  ...
  List<String> _getVisibleCategories(RideCategory driverCategory) {
    switch (driverCategory) {
      case RideCategory.best:
        return ['best', 'family', 'energy', 'standard'];
      case RideCategory.energy:
        return ['energy', 'standard'];
  ...
  Future<List<Ride>> _processRidesInBackground(QuerySnapshot snapshot) async {
    final position = await _getCachedPosition();
    if (position == null) return [];

    final allRides = snapshot.docs.map((doc) => Ride.fromFirestore(doc as DocumentSnapshot<Map<String, dynamic>>)).toList();
  ...
  Future<void> updateDriverLocation(geolocator.Position position, {double? bearing}) async {
    if (_uid == null) return;

    _locationDebouncer?.cancel();
    _locationDebouncer = Timer(const Duration(milliseconds: 500), () {
  ...
  Future<void> addStopToRide(String rideId, StopLocation newStop) async {
    ...
    Future.microtask(() async {
      try {
        ...
        final newFareData = _pricingService.calculateFareWithStops(
          distanceInKm: newDistanceKm,
          durationInMinutes: newDurationMinutes,
          category: ride.category,
          numberOfStops: updatedStopsMap.length,
        );
    ...
    Future.microtask(() async {
      try {
        ...
        final newFareData = _pricingService.calculateFareWithStops(
          ...
          durationInMinutes: newDurationMinutes,
          category: ride.category,
          numberOfStops: ride.stops.length,
        );
  ...
  Future<void> passengerConfirmDriver(String rideId) async {
    _addToBatch('ride_requests', rideId, {'status': 'accepted'});
  }
  ...
  Stream<Ride?> getActiveDriverRideStream() {
    ...
    return _db.collection('ride_requests')
        ...
        .where('status', whereIn: ['driver_found', 'accepted', 'arrived', 'in_progress'])
        .limit(1)
        .snapshots()
        .map((snapshot) {
  ...
  Stream<List<Ride>> getDriverRidesHistory({DateFilter filter = DateFilter.all}) {
    ...
    // ✅ BACKWARD COMPATIBILITY: Pentru cursele vechi care nu au driverId setat corect
    // Caută toate cursele completed și filtrează în client
    return _db
        .collection('ride_requests')
        ...
        .snapshots()
        .map((snapshot) {
          List<Ride> allRides = snapshot.docs
              .map((doc) => Ride.fromFirestore(doc as DocumentSnapshot<Map<String, dynamic>>))
              .toList();
          
          // ✅ FILTREAZĂ ÎN CLIENT: Doar cursele unde utilizatorul a fost șofer
          List<Ride> driverRides = allRides
              .where((ride) => ride.driverId == _uid)
              .toList();
  ...
  Future<Map<String, dynamic>> validateVoucher(String code) async {
    ...
    final validCodes = {
      "FRIENDS10": {"success": true, "message": "10% voucher applied successfully!"},
      "PROMO5": {"success": true, "message": "5 RON discount applied successfully!"},
    };
  ...
  Future<String> createRideRequest(RideRequest rideRequest) async {
    ...
    try {
      ...
      rideData['passengerId'] = _uid;  // ✅ Asigură-te că folosește ID-ul corect
      rideData['timestamp'] = FieldValue.serverTimestamp();
      rideData['searchRadius'] = 5.0;
      rideData['searchStartTime'] = FieldValue.serverTimestamp();
      rideData['distance'] = distance; // ✅ Actualizează distanța calculată
      ...
      // ✅ ERROR HANDLING: Start driver search în background (nu blochează)
      try {
        unawaited(_startAutomaticDriverSearchForRideRequest(docRef.id, rideRequest).catchError((e) {
          Logger.error('Error starting automatic driver search', error: e, tag: 'FIRESTORE');
        }));
      } catch (e) {
        Logger.warning('Error starting driver search: $e', tag: 'FIRESTORE');
        // Nu aruncăm eroare - ride-ul a fost creat, doar driver search-ul a eșuat
      }
  ...
  Future<void> _startAutomaticDriverSearchForRideRequest(String rideId, RideRequest rideRequest) async {
    ...
    try {
      ...
      // Pentru moment, doar setează status-ul la 'searching'
      await _db.collection('ride_requests').doc(rideId).update({
        'status': 'searching',
  ...
  Stream<List<Ride>> getRidesHistory({DateFilter filter = DateFilter.all, int? limit}) {
    ...
    // ✅ BACKWARD COMPATIBILITY: Pentru cursele vechi care au userId în loc de passengerId
    // Caută toate cursele completed și filtrează în client
    return _db
        .collection('ride_requests')
        .where('status', isEqualTo: 'completed')
        ...
        .map((snapshot) {
          ...
          // ✅ FILTREAZĂ ÎN CLIENT: Doar cursele unde utilizatorul a fost pasager
          List<Ride> passengerRides = allRides
              .where((ride) => ride.passengerId == _uid)
              .toList();
  ...
  Stream<List<Ride>> getPassengerScheduledRides() {
    if (_uid == null) return Stream.value([]);
    return _db
        .collection('ride_requests')
        .where('passengerId', isEqualTo: _uid)  // ✅ MODIFICAT: userId → passengerId
        .where('isScheduled', isEqualTo: true)
        .where('status', whereIn: ['pending', 'driver_found', 'accepted', 'arrived'])
  ...
  Future<void> cancelRide(String rideId) async {
    ...
    // ✅ Pentru curse programate: verifică dacă anularea este înainte de 1.5 ore
    if (ride.isScheduled && ride.scheduledPickupTime != null) {
      final timeUntilPickup = ride.scheduledPickupTime!.difference(now);
      
      // Taxă dacă mai sunt sub 1.5 ore (90 minute) până la pickup
      if (timeUntilPickup.inMinutes < 90 && timeUntilPickup.inMinutes > 0) {
        // Taxa este 30 RON sau prețul preafișat al cursei (se va debita valoarea cea mai mică)
        fee = math.min(30.0, ride.totalCost);
      }
      // Anulare gratuită dacă mai sunt cel puțin 1.5 ore (90 minute) până la pickup
    } else {
  ...
  /// ✅ NOU: Curăță automat cursele blocate (verifică toate cursele, nu doar pentru user-ul curent)
  Future<void> _autoCleanupStuckRides() async {
    try {
      ...
      // ✅ Găsește toate cursele blocate (searching, driver_found, accepted, etc.) care sunt mai vechi de 10 minute
      // Status "searching" este inclus pentru că poate bloca crearea de noi curse
      final stuckRides = await _db.collection('ride_requests')
          .where('status', whereIn: ['searching', 'driver_found', 'accepted', 'arrived', 'in_progress'])
          .get();
      ...
      for (final doc in stuckRides.docs) {
        ...
        // ✅ Verifică dacă cursa este blocată (mai veche de 10 minute)
        // Pentru status "searching", folosim un prag mai scurt (5 minute) pentru a permite testarea mai rapidă
        final status = data['status'] as String?;
  ...
  Future<void> resetStuckRides() async {
    ...
    try {
      ...
      // ✅ Găsește cursele blocate pentru pasager (inclusiv "searching" care poate bloca crearea de noi curse)
  ...
  Future<void> forceCleanupAllActiveRides() async {
    ...
    try {
      ...
      final allActiveRides = await _db.collection('ride_requests')
          .where('status', whereIn: ['driver_found', 'accepted', 'arrived', 'in_progress', 'pending'])
          .get();
      
      final batch = _db.batch();
      int cleanedCount = 0;
      
      for (final doc in allActiveRides.docs) {
        final data = doc.data();
...
class SafeFirestoreOperations {
  ...
  static void cacheQuery(String queryKey, dynamic result) {
    ...
    Duration ttl = Duration(minutes: 5); // Default TTL
    if (queryKey.contains('user_profile')) {
      ttl = Duration(minutes: 30); // User profiles cache longer
    } else if (queryKey.contains('driver_location') || queryKey.contains('real_time')) {
      ttl = Duration(seconds: 30); // Real-time data cache shorter
    } else if (queryKey.contains('static') || queryKey.contains('poi')) {
      ttl = Duration(hours: 1); // Static data cache much longer
    }
  ...
  static bool canExecuteQuery(String queryKey) {
    ...
    if (queryKey.contains('driver_location') || queryKey.contains('real_time')) {
      minInterval = Duration(milliseconds: 500); // More frequent for real-time data
    } else if (queryKey.contains('user_profile') || queryKey.contains('static')) {
      minInterval = Duration(seconds: 5); // Less frequent for static data
    }
  ...
  static Future<T?> safeFirestoreOperation<T>(
    ...
    final timeout = customTimeout ?? Duration(seconds: 30);
    ...
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        Logger.error('Firebase error for $operation: ${e.code}', error: e, tag: 'FIRESTORE', context: {'message': e.message});
